...
module.exports = {
  ...
  run: async (client, message, args) => {
        ...
        if(!target) return message.channel.send('<a:TP_Check_No:896786444507574313> No member specified') //when no member is pinged
        const role = message.mentions.roles.first() // roles = mentions
        if(!role) return message.channel.send('<a:TP_Check_No:896786444507574313> No role specified') //when no role is specified or pinged
        //now the code!
